 ...
 */
	public static function custom($check, $regex = null) {
		...
		if ($regex === null) {
			self::$errors[] = __d('dev', 'You must define a regular expression for %s', 'Validation::custom()');
			return false;
		}
 ...
 */
	protected static function _pass($method, $check, $classPrefix) {
		...
		if (!class_exists($className)) {
			trigger_error(__d('dev', 'Could not find %s class, unable to complete validation.', $className), E_USER_WARNING);
			return false;
		}
		if (!method_exists($className, $method)) {
			trigger_error(__d('dev', 'Method %s does not exist on %s unable to complete validation.', $method, $className), E_USER_WARNING);
			return false;
		}
		$check = (array)$check;
 ...
 */
	public static function mimeType($check, $mimeTypes = array()) {
		...
		if ($mime === false) {
			throw new ExceptionHelper(__d('cake_dev', 'Can not determine the mimetype.'));
		}
