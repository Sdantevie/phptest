...
class Timezone  {
    ...
    /**
     * Get the timezone list and translate it to html select opetion
     *
     * @param   string     $val
     * @return  html|option
     */
     public static function get_timezone_option($val=''){

        // check if an array of the user selcted country is available
        if(!empty($val)):
            $key = array_search ($val, self::get_timezone_list());
            if($key){
             echo "<option selected value=".trim($val).">".$key."</option>";    
            } 
        endif;

          foreach (self::get_timezone_list() as $k => $v) {
