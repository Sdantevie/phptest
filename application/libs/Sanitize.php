 ...
 */
	...
	public static function remove_quote($data){
           $data = str_replace("'", " ", $data);
           return $data;
    }

    private static function remove_doublewhitespace($s = null){
           return  $ret = preg_replace('/([\s])\1+/', ' ', $s);
    }

    private static function remove_whitespace($s = null){
           return $ret = preg_replace('/[\s]+/', '', $s );
    }

    private static function add_br_html( $s = null){
           return $ret = preg_replace('/[\n]/', '<br>', $s);
    }

    private static function remove_br_html( $s = null){
           return $ret = preg_replace('/<br>/', '
          										',  $s);
    }

    public static function stripCommas($s){
    	$ret1 = str_replace(array('.', ','), '' , $s);
    	$ret .= preg_replace('/\s\s+/', ' ', $ret1);
    	 return trim($ret);
	...
	public static function cleanAll($data){
           $data = self::remove_quote(self::stripAll($data));

           return $data;
    }

/* The smart clean is meant to simplify the task of sanitizing a rendered post
*/
    public static function smart_add_html($s = null){
           return $ret = trim( self::remove_doublewhitespace( self::add_br_html($s) ) );
    }

     public static function smart_clean_html($s = null){
           return $ret = trim( self::remove_doublewhitespace( self::remove_br_html($s) ) );
    }
